...
for i in range(len(landMarkedShapes)):
    ...
    translationV = tdict['translation']
    
    r = np.repeat(translationV[0],30)
    r2 = np.repeat(translationV[1],30)
    tran= np.vstack((r,r2))
    
    translatedShape = (np.add(np.dot(rotationM,landMarkedShapes[i].T),tran)).T
    listTranslated.append(translatedShape)
